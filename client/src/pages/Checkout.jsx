    ...
    const handleRazorpayPayment = async (backendResponse) => {
        ...
        const options = {
            ...
            // ✅ FIX: Modal Close Hone Par Pending Order Delete Karo
            modal: { 
                ondismiss: async function() { 
                    setLoading(false); 
                    toast.warning("Payment Cancelled. Order not placed."); 
                    
                    // 🔥 IMPORTANT: Backend ko batao ki payment nahi hui, order uda do
                    try {
                        await api.delete(`/orders/${order._id}`);
                        console.log("Pending order deleted due to cancellation");
                    } catch (err) {
