...
import {
  ...
  ShoppingCart,
  Search,
  ClipboardList, // ✅ Imported for Orders
} from "lucide-react";
    ...
    <>
      ...
      <div className="min-h-screen bg-[#020202] flex flex-col font-manrope selection:bg-yellow-500/30 selection:text-yellow-100 relative">
        ...
        <header className="bg-black/80 border-b border-yellow-600/20 sticky top-0 z-40 backdrop-blur-md shadow-[0_4px_30px_-10px_rgba(212,175,55,0.1)]">
            ...
            <div className="flex items-center justify-between h-20">
              ...
              <div className="hidden md:flex flex-1 max-w-md mx-8">
                <div className="relative w-full group">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-[#0a0a0a] border border-yellow-600/20 rounded-lg text-yellow-50 placeholder-gray-600 focus:outline-none focus:border-yellow-500/50 transition-all shadow-inner text-gray-200"
                  />
                  {localSearchQuery && (
                    <button
                      onClick={() => {
              ...
              {isAdmin && (
                <nav className="hidden md:flex items-center gap-8">
                  <Link to="/dashboard" className="text-gray-400 hover:text-yellow-400 transition-colors text-xs uppercase tracking-widest font-semibold relative group">
                    Dashboard
                ...
                <div className="hidden md:block relative">
                  <button
                    ...
                    className={`flex items-center gap-3 px-3 py-1.5 rounded-full border transition-all duration-300 ${
                      isProfileOpen ? "bg-yellow-900/10 border-yellow-500/50" : "bg-transparent border-transparent hover:border-yellow-600/30"
                    }`}
                  >
                  ...
                  {isProfileOpen && (
                    <>
                      ...
                      <div className="absolute right-0 mt-3 w-64 bg-[#0a0a0a] border border-yellow-600/30 rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.9)] backdrop-blur-xl z-20 overflow-hidden">
                        
                        {/* User Info Header */}
                        <div className="p-5 border-b border-white/5 bg-gradient-to-r from-yellow-900/10 to-transparent">
                          <p className="text-sm font-cinzel font-bold text-yellow-100">{name || "User"}</p>
                          <p className="text-xs text-gray-500 mt-1">{email || "No email"}</p>
                        ...
                        <div className="p-2 space-y-1">
                          {isGuest && (
                            <Link 
                                to="/login" 
                                className="flex items-center gap-3 px-3 py-2 text-xs font-medium text-yellow-400 hover:bg-white/5 rounded-lg"
                          ...
                          {!isGuest && (
                            <Link 
                                to="/orders" 
                                className="flex items-center gap-3 px-3 py-2 text-xs font-medium text-gray-300 hover:bg-white/5 rounded-lg transition-colors"
                                onClick={() => setIsProfileOpen(false)}
                            >
                          ...
                          <button 
                            ...
                            className="w-full flex items-center gap-3 px-3 py-2 text-xs font-medium text-red-400 hover:bg-red-900/10 rounded-lg transition-colors"
                          >
          ...
          {/* MOBILE MENU */}
          {isMobileMenuOpen && (
            <div className="md:hidden border-t border-yellow-600/20 bg-[#050505]/95 backdrop-blur-xl">
              <div className="px-4 py-6 space-y-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={localSearchQuery}
                    onChange={handleSearchChange}
                    className="w-full pl-10 pr-10 py-3 bg-[#0a0a0a] border border-yellow-600/20 rounded-lg text-yellow-50 placeholder-gray-600 focus:outline-none focus:border-yellow-500/50 transition-colors text-sm text-gray-200"
                  />
                </div>
                ...
                {!isGuest && (
                    <Link 
                        to="/orders" 
                        className="flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-yellow-400 hover:bg-white/5 rounded-lg transition-colors text-sm font-medium border-l-2 border-transparent hover:border-yellow-500"
                        onClick={() => setIsMobileMenuOpen(false)}
                    >
                        <ClipboardList className="w-4 h-4 text-yellow-500" /> My Orders
                    </Link>
                ...
                <div className="pt-4 border-t border-white/10">
                  <div className="px-2 py-2 mb-3">
                    <p className="text-sm font-cinzel font-bold text-white">{name || "User"}</p>
